...
def adivina_numero():
    if request.method == 'GET':
        msg = 'Adivina un número entre 1 y 100'
        exito = False
        session['num_pc'] = random.randint(1,100) # usamos session para que guarde el valor entre una request y otra
        session['intentos'] = 0
    else:
        ...
        if num == session['num_pc']:
            ...
            exito = True
            msg = msg + ' -- ' + insert_adivina_numero(datetime.now().strftime("%Y-%m-%d %H:%M:%S"),num_intentos)
        else:
            exito = False
            if num > session['num_pc']:
                msg = 'Demasiado alto'
